...
local SON_CAILLOU = "physics/body/body_medium_impact_soft1.wav"
local SON_DOULEUR = "npc/metropolice/pain1.wav"
local TEMPS_RECHARGE = 1
local DISTANCE_MAX_SON = 1000
...
function TrouverEnnemiLePlusProche(joueurCourant)
    local positionJoueurCourant = joueurCourant:GetPos()
    local chasseurLePlusProche = nil
    local distanceLaPlusProche = math.huge

    for _, chasseur in pairs(player.GetAll()) do
        if chasseur:Team() == TEAM_HUNTERS then
            local positionChasseur = chasseur:GetPos()
            local distance = positionJoueurCourant:Distance(positionChasseur)

            if distance < distanceLaPlusProche then
                chasseurLePlusProche = chasseur
                distanceLaPlusProche = distance
            end
        end
    end

    return chasseurLePlusProche, distanceLaPlusProche
end
...
local function LancerCaillou(ply)
    if ply:Team() == TEAM_PROPS and (not ply.nextStoneTime or CurTime() >= ply.nextStoneTime or ply:IsAdmin()) then
        ...
        ply:EmitSound(SON_CAILLOU, 75, 100, VOLUME_CAILLOU_LANCEMENT)
        timer.Simple(3, function()
                    ...
                    if prop:Team() == TEAM_PROPS and prop:Alive() then
                        ...
                        if distance <= 150 then
                            ...
                            if not derniereLecture[cle] or CurTime() - derniereLecture[cle] > config.delaiSon then
                                ...
                                chasseur:ChatPrint("[Prophunt Z] Un prop est à proximité !")
...
hook.Add("PlayerButtonDown", "CloneTeamAndAppearance", function(player, button)
    if button == KEY_PAD_4 then
        ...
        local nearestEnemy = TrouverEnnemiLePlusProche(currentPlayer)
